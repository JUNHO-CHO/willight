...
#     answer = len(nums)
#     p_count = answer//2
#     all_types = len(set(nums))
#     max_nums = min(p_count, all_types)
#
#     return max_nums
...
한글 풀이
여러 종류의 포켓몬은 N/2 선택
N/2마리의 포켓몬을 선택하는 방법 중 가장많은 종류의 포켓몬 선택하는 방법찾기
사용할 함수
1. set()
2. min()
3. len()
...
def solution(nums):
    answer = 0
    a = len(nums)            #a는 리스트 nums의 길이
    b = a//2                 #b는 a를 2로 나누고 나머지는 버린값
    a_b = len(set(nums))     #a_b는 nums의 집합요소의 길이
    max_ab=min(a,a_b)        #max_ab는 a와 a_b의 최솟값
    return max_ab            #max_ab값 반환
...
def solution(nums):
    ...
    b = a//2                 #b는 a를 2로 나누고 나머지는 버린값
    a_b = len(set(nums))     #a_b는 nums의 집합요소의 길이
    max_ab=min(b,a_b)        #max_ab는 b와 a_b의 최솟값
    return max_ab
